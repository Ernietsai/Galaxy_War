...
root.geometry(f'+{root.winfo_screenwidth()}+0')
if is_fullscreen:
    width = root.winfo_screenwidth()  #500
    higth = root.winfo_screenheight()  #600
else:
    width = 500
    higth = 600
...
screen = pygame.display.set_mode((width,higth), flags = pygame.constants.FULLSCREEN if is_fullscreen else 0)
...
def start_ad(surf):  #廣告
    ...
    surf.blit(ad_img, (width/2 - ad_img_size[0]/2, higth/2 - ad_img_size[1]/2))
...
class Star(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect.bottom = random.randint(-higth*2, 0)#(-higth, higth)
        self.rect.centerx = random.randint(1, width-1)
    
    def update(self):
        self.rect.centery += star_speed
        if self.rect.top > higth:
            self.size = random.uniform(2, 3.5)
            self.image = pygame.Surface((self.size, self.size))
            self.rect = self.image.get_rect()
            pygame.draw.circle(self.image, white, self.rect.center, self.size)
            self.rect.bottom = random.randint(-higth, 0)
            self.rect.centerx = random.randint(1, width-1)
...
background     = pygame.transform.scale(background,(width,higth))
menu_background     = pygame.image.load(os.path.join('picture',"遊戲圖示(大).png")).convert()
menu_background     = pygame.transform.scale(menu_background,(width,higth))
...
def draw_blood(surf, blood, x, y, color):
    ...
    BAR_LENTH = 100
    BAR_HIGTH = 10
    blood_rect = pygame.Rect(x, y, BAR_LENTH*(blood/FULL_BLOOD), BAR_HIGTH)
    full_rect = pygame.Rect(x, y, BAR_LENTH, BAR_HIGTH)
...
class Star(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        pygame.draw.circle(self.image, white, self.rect.center, self.size)
        self.rect.bottom = random.randint(-higth, higth)
        self.rect.centerx = random.randint(1, width-1)
...
while running:
    ...
    for event in pygame.event.get():#鍵盤事件和關閉
        ...
        if event.type == pygame.KEYDOWN:  #按鍵盤
            ...
            if event.key == pygame.K_ESCAPE:  #按 Esc 鍵
                channel.pause()
                root.deiconify()  #顯示 root 視窗
                if messagebox.askokcancel('離開', '確定要離開 Galaxy war 嗎?'):  #離開
                    running = False
                else:
                    channel.unpause()
                root.withdraw()  #隱藏 root 視窗
